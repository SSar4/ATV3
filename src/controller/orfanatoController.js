...
module.exports = {
    async index(request, response, next){
       ...
       const message = await conection('orfanato').limit(10).
       offset((page -1)*10).
       select('id','nome');
      ...
      async findOne(request, response,next){
        ...
        const message = await conection('orfanato').
        where('id',id).select('nome','sobre','telefone','intrucoes','h_visita','fim_semana');
         response.json({message})
     },
     async create(request, response, next){
     
        const {nome,sobre,telefone,instrucao,h_visita,fim_semana } = request.body;
